...
import { UserEntityForCU } from 'navmenu/models';
...
function Register() {
	...
	let onRegister = () => {
		update(() => {
			...
			entity.slapyvardis = state.username;
			entity.slaptazodis = state.password;
			entity.el_pastas = state.email;
			...
			>
				...
				<div className="mb-3">
					...
					<InputText
						...
						placeholder="?veskite el_pašt?"
						autoFocus
						value={state.email}
						onChange={(e) => update(() => state.email = e.target.value)}
					/>
					{state.isEmailErr &&
						<div className="invalid-feedback">?veskite el_pašt?</div>
					}
				</div>
				<div className="mb-3">
					<label
